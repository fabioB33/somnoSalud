...
**Recomendación inmediata:** pedirle a Pablo el repo `PaulFerrero/somnosalud` actual + cualquier carpeta local con los `.ts` originales antes de migrar. Si NO tiene los `.ts` (porque trabajó en Lovable.dev y solo bajó el bundle), tenemos que reconstruir desde el bundle (factible pero más lento).
...
|---|---|---|---|---|
| 0.1 | Pedirle a Pablo el repo `PaulFerrero/somnosalud` actual + carpetas locales con `.ts` source | Jorge | 30 min | — |
...
1. **Acceso al repo `PaulFerrero/somnosalud`** (ahora) — agregame como collaborator @jorgeleporace si está privado.
