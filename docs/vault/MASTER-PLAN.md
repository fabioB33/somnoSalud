...
- ✅ **Tier 2 paridad webapp ↔ mobile — Sprints 10-14 mergeados en `main` (2026-06-19):** ola de paridad para que la webapp alcance las features que la app mobile ya tenía. **5 sprints, todos con código en `main` + tests verdes**, mergeados secuencialmente. Verificados empíricamente 2026-06-19 (rutas existen + 49/49 tests webapp passing + build 30/30 páginas):
  - **Sprint 10 — Plan + Tab Hoy** (commit `a670201`, merge `7b6bde9`): `/hoy` con plan diario derivado del último resultado + checks diarios (`plan-builder.ts` + `TodayPlan.tsx`). Continuidad post-`/eval/results`. Frontmatter aún `code-pending-merge` (drift de status — el código está en main).
  - **Sprint 11 — Diary tracking + Gamification** (commit `d2b2a2e`, merge `47df125`): `/diario` + `/progreso` con `diary_entries` (migration 0006) + streaks + insights longitudinales (`progress-builder.ts`). Frontmatter `code-pending-merge` (drift).
  - **Sprint 12 — Backoffice clinician multi-user** (commit `83a9e24`, merge `8e5f063`): `/backoffice` con `clinician_links` (migration 0008) + RLS multi-tenant. **Crítico para venta clínica** — Pablo puede ver pacientes desde su laptop para mostrar a colegas IFN. Frontmatter `code-pending-merge` (drift).
  - **Sprint 13 — Premium waitlist P8** (commit `10b1245`, merge `83fc65a`): `/premium` con `premium_waitlist` (migration 0007) + join/leave waitlist. **OJO drift de numeración:** este "Sprint 13" (Tier 2 paridad) es DISTINTO al "Sprint 13 = E2E Playwright" listado más arriba en Fase 1. Dos numeraciones colisionan (Tier 2 paridad-mobile vs Fase 1 original). Frontmatter `code-pending-merge` (drift).
  - **Sprint 14 — Mis resultados sub-pantallas P5.1-P5.5** (commit `bbbd739`, merge `c494cab` = HEAD actual): `/mis-resultados/[id]/insights/[insight]` con `insights-builder.ts` (P5.1-P5.5: condensado por instrumento + resumen). **CIERRE Tier 2.** Mismo drift de numeración vs "Sprint 14 = Observabilidad" de Fase 1.

- ✅ **Deploy sync mirror Vercel (2026-06-19):** el mirror `fabioB33/somnoSalud` (que Vercel deploya) estaba **18 commits atrasado** (`a5c6747`, "refresh visual post-feedback Pablo"). Producción servía una versión sin los Sprints 9.G → 14. Fix: `git push vercel main` fast-forward limpio (verificado `git merge-base --is-ancestor`: vercel/main era ancestro directo, sin divergencia, sin `--force`). Mirror sincronizado a `c494cab` = idéntico a `origin/main`. Auto-deploy Vercel disparado. **Pendiente verificación Fabio:** dashboard Vercel deploy `Ready` + Supabase Redirect URLs intactas. Detalle: [[sessions/2026-06-19-LOG-auditoria-codigo-faltante-deploy-sync]].

- ✅ **Auditoría "¿falta código?" (2026-06-19):** triangulación empírica E1+E2+E3 (3 agentes paralelos + build/test reales) para responder a Fabio si faltaba código antes de mostrar a cliente. **Veredicto: NO falta código.** 227 tests passing (55 clinical-engine + 104 psg-parser + 19 conversor + 49 webapp), typecheck webapp 0 errores, build producción `Compiled successfully` + 30/30 páginas. El único "error" de build local era falta de `.env` (env-only, NO código) — confirmado creando `.env.local` dummy → build 30/30 OK. Únicos no-completos (ninguno bloqueante): `shared-ui` skeleton intencional (`build` = echo) + Sprint 20 research sin código. Resend/Sentry son `idle` esperando API key = operación, no código faltante. Detalle: [[sessions/2026-06-19-LOG-auditoria-codigo-faltante-deploy-sync]].

- 📋 **Sprint 20 research-pending-approval (2026-06-02):** Agente conversacional de voz para reemplazar 6 cuestionarios múltiple choice (ISI/ESS/STOP-BANG/PHQ-9/GAD-7/DASS-21 = 57 preguntas). **Origen:** decisión Jorge + Pablo 2026-06-02 (WhatsApp): "decidimos poner un agente que reemplace las preguntas tipo múltiple choice, con ElevenLabs o con Vapi". **Status:** sin código, solo research + decisión técnica + plan FASE 1-4. **Research comparativo Vapi vs ElevenLabs** con fuentes empíricas (vapi.ai/pricing, elevenlabs.io/pricing). **Recomendación técnica:** Vapi para Fase 1 ($0.05/min + BYO Claude API key + HIPAA add-on $2000/mo predictable vs ElevenLabs custom enterprise). **MVP propuesto:** 1 cuestionario piloto (PHQ-9 por ítem 9 LIVE detection ya implementado). **7 decisiones bloqueantes pendientes** (Vapi vs ElevenLabs + scope MVP + consent voz + retención audio + LLM + qué es "base de conocimiento" mencionada por Jorge). Detalle: [[sprints/sprint-20-agente-conversacional/SPRINT-20-AGENTE-CONVERSACIONAL]].

**Próxima acción inmediata (post auditoría 2026-06-19):**
1. **Verificar deploy Vercel `Ready`** (post sync mirror) — Fabio en dashboard. El commit `c494cab` trae 18 commits con deps nuevas (Sentry +83 KB, Resend, framer-motion); confirmar que el build de producción salió OK + Supabase Redirect URLs intactas.
2. **Reconciliar frontmatter Sprints 10-14** de `code-pending-merge` → `closed-verified` (el código ya está en main + tests verdes — es solo drift de status). Bloque K del [[processes/SPRINT-CLOSURE-CHECKLIST]].
3. **Sprint Resend SMTP custom** (~2h): el dominio `somnosalud.com.ar` **NO está registrado** (verificado NXDOMAIN 2026-06-19). Pablo tiene un dominio → decidir sender (subdominio `ifn.com.ar`, registrar `.com.ar`, o `pampalabs.com`) antes de avanzar. Runbook listo: [[runbooks/RUNBOOK-resend-smtp-supabase-activation]].
4. **Sprint 20** (agente voz): bloqueante hasta signoff Jorge+Pablo de las 7 decisiones.
5. **Sprint 3.B Conversor PSG a Vercel separado** (~2h, requires smoke Pablo con PDFs reales).
...
*Última actualización: 2026-06-19 (Tier 2 paridad Sprints 10-14 documentados en roadmap + deploy mirror Vercel sincronizado 18 commits + auditoría "¿falta código?" empírica = NO falta código, 227 tests verdes)*
*last_synced_with_vault_reality: 2026-06-19*
*Próxima revisión: post-reconciliación frontmatter Sprints 10-14 + post-Sprint Resend SMTP (destrabar rate limit) o Sprint 20 agente voz si llega signoff*
