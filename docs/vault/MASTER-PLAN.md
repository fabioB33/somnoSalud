...
**Próxima sesión sugerida:** Sprint 8 — `/eval/results` (Capa 5 ADR-003): invocar `scoreISI/ESS/STOPBANG/PHQ9/GAD7/DASS21` + `classifyInsomniaPhenotype` + `assessRisk` + `generateRecommendations` + `calculatePrecision` + `analyzeLabPanel` + `analyzeGeneticProfile`. Disclaimer reforzado + M.N. + export PDF + reset. Estimado 4-5h.
