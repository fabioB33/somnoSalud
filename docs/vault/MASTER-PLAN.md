...
- 🚀 **Sprint 7.A in-progress (2026-05-09):** Capa 4 ADR-003 (`/eval/safety` con `evaluateAllSafetyRules` del clinical-engine + bloqueo hard / restrict warning) + componente genérico `<QuestionnaireForm>` reutilizable + 3 cuestionarios (ISI, ESS, STOP-BANG). Detalle: [[sprints/sprint-7-a-cuestionarios-safety/SPRINT-7-A-CUESTIONARIOS-SAFETY]].

**Próxima sesión sugerida:** continuar Sprint 7.A → Sprint 7.B (PHQ-9 con detección ítem 9 + GAD-7 + DASS-21 + sleep + lab + genetics opcionales).
