...
- 🚀 **Sprint 6 in-progress (2026-05-08):** implementa capas 1-3 de [[architecture/adr/ADR-003-compliance-gates-en-codigo|ADR-003]] (middleware + DisclaimerBanner layout + verificación edad <18) + 4 pantallas P0 (/disclaimer, /terms, /eval/profile, /eval/menor-no-permitido) + estado client-side (sessionStorage + cookie consent) + 4 componentes shadcn nuevos (Checkbox, Input, Label, Alert). Detalle: [[sprints/sprint-6-compliance-gates/SPRINT-6-COMPLIANCE-GATES]].

**Próxima sesión sugerida:** continuar Sprint 6 → Sprint 7 (cuestionarios ISI + STOP-BANG + PHQ-9 + GAD-7 + DASS-21 + sleep + lab + genetics, capa 4 safety rules).
