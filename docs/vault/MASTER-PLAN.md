...
- 🚀 **Sprint 8 in-progress (2026-05-09):** `/eval/results` (Capa 5 ADR-003) — última pantalla del flow Fase 1 client-side. Invoca todos los `score*` del clinical-engine + `classifyInsomniaPhenotype` + `assessRisk` + `generateRecommendations` (respetando safety rules) + `calculatePrecision` + `analyzeLabPanel` + `analyzeGeneticProfile`. `<DisclaimerBanner variant="reinforced">` arriba/abajo + M.N. visible + secciones colapsables (shadcn Accordion) + export PDF (`window.print()` + CSS `@media print`) + reset (limpia sessionStorage). Detalle: [[sprints/sprint-8-results-capa-5/SPRINT-8-RESULTS-CAPA-5]].

**Próxima sesión sugerida:** Sprint 9+ — Supabase setup (necesita Sprint 2.B Fabio) + auth + persistencia migrada de sessionStorage a `evaluations` table con RLS multi-tenant. Cierra Fase 1.
