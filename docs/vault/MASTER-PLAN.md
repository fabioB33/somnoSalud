...
- 🚀 **Sprint 8.6 in-progress (2026-05-09):** Welcome expandida + 3 páginas estáticas públicas + footer compartido. `app/page.tsx` rewrite con secciones "Cómo funciona" + "Qué vas a recibir" + FAQ accordion. `/about` con info Dr. Ferrero M.N. + IFN + compliance regulatorio resumido. `/privacidad` con política completa Ley 25.326/26.529/Decreto 1089/2012. `/not-found` 404 custom. `<PublicFooter>` reusable. Detalle: [[sprints/sprint-8-6-welcome-expandida/SPRINT-8-6-WELCOME-EXPANDIDA]].

**Próxima sesión sugerida:** smoke visual humano del Sprint 8.6 + Sprint 9 (Supabase setup, requiere Sprint 2.B Fabio) o tests E2E Playwright (Sprint 13).
