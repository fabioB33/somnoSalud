...
- [[sprints/sprint-8-results-capa-5/SPRINT-8-RESULTS-CAPA-5]] — Sprint 8 implementa Capa 5 ADR-003 con `/eval/results` real: scoring de los 6 instrumentos + phenotype + risk + recommendations (respetando safety rules) + precision + lab/genetics opcionales + DisclaimerBanner reinforced + Accordion colapsable + export PDF (window.print + CSS print media) + reset. Cierra **Fase 1 client-side**. Status: in-progress.
