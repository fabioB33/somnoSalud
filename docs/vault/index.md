...
- [[sprints/sprint-8-6-welcome-expandida/SPRINT-8-6-WELCOME-EXPANDIDA]] — Sprint 8.6 welcome expandida (cómo funciona + qué vas a recibir + FAQ accordion) + 3 páginas estáticas públicas: `/about` (Dr. Ferrero + IFN + compliance), `/privacidad` (política Ley 25.326/26.529 completa), `/not-found` (404 custom). Footer compartido `<PublicFooter>` reusable. Status: in-progress.
