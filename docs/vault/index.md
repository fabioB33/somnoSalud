...
- [[sprints/sprint-19-b-engine-ui-zip/SPRINT-19-B-ENGINE-UI-ZIP]] — Sprint 19.B features iterativas Conversor PSG. **`buildZip()`** con JSZip + BOM UTF-8 + filename `CSV_PSG_<timestamp>.zip`. **`<EnginePanel>`** React (~280 LOC) con 2 tabs: Resultados (score grande coloreado por catClass + 12 metric cards + 6 breakdown bars + perfil A/B/C + flags clínicos + tabla detalle 11 filas) y Methodology (7 secciones del scoring + cita Azarbarzin 2019 con DOI link). Integración: `<FileRow>` botón "Score Hipóxico" condicional + `<App>` state engineFileId + render condicional panel + botón "Descargar todos (ZIP)". **19/19 tests vitest** (16 CSV + 3 ZIP). Vite build verde 4.11s. **Total monorepo: 178 vitest tests**. **Progreso Conversor PSG: 95% (8.5/9)** — solo falta Sprint 19.C archivar legacy con smoke real de Pablo. Status: closed-verified.
- [[sprints/sprint-19-c-archivar-legacy/SPRINT-19-C-ARCHIVAR-LEGACY]] — Sprint 19.C archivado formal `legacy-v0/index.html` (94 KB, 1.887 LOC HTML monolítico) a `legacy-v0/_archived/` con `git mv` (historial git preservado). README archivado nuevo con: explicación del cambio + tabla migraciones Sprint 15-19.B + instrucciones de recovery (`git mv` reverse en <5s) + smoke comparativo con `python3 -m http.server`. `package.json` description + README del package + 7 docstrings en `src/` (csv, filename, pdf, schema, styles, zip, EnginePanel, App) actualizados al path nuevo. Verificación: typecheck verde + 19/19 vitest + Vite build 2.88s + cero refs stale. **Cierra DEBT-conversor-psg-migration-roadmap (closed-verified 2026-05-26)**. Migración Conversor PSG **100% completa** (Sprints 15-19.C, ~20.5 h totales). Status: closed-verified.
- [[sprints/sprint-3-vercel-preview/SPRINT-3-VERCEL-PREVIEW]] — Sprint 3 webapp-somnosalud live en **producción Vercel**. URL `https://somno-salud-webapp-somnosalud.vercel.app`, repo mirror `fabioB33/somnoSalud` con auto-deploy on push to main. **3 usuarios reales registrados + 2 logueados verificados vía admin API** (Fabio + Pablo entraron OK end-to-end; Jorge rate-limited antes de completar). Smoke HTTP 7 rutas con codes esperados. **Hotfix mid-sprint** `9527b55`: bug magic link redirigía a localhost → fix defense-in-depth con `lib/site-url.ts` (precedencia `NEXT_PUBLIC_SITE_URL` → `VERCEL_PROJECT_PRODUCTION_URL` → `VERCEL_URL` → header origin → localhost fallback) + corrección Site URL en Supabase Auth dashboard. **DEBT-resend-smtp-supabase elevado a `high`** post-smoke (rate limit 4 emails/h chocó múltiples veces durante debug del hotfix). Status: closed-verified.
- [[sprints/sprint-20-agente-conversacional/SPRINT-20-AGENTE-CONVERSACIONAL]] — Sprint 20 research + decisión técnica para agente conversacional de voz que reemplace los 6 cuestionarios MC (ISI+ESS+STOP-BANG+PHQ-9+GAD-7+DASS-21 = 57 preguntas). **Origen:** decisión Jorge+Pablo 2026-06-02 WhatsApp. **Sin código** — solo research empírico (Vapi vs ElevenLabs con fuentes documentadas) + plan FASE 1-4 + hipótesis + 7 decisiones bloqueantes pendientes. **Recomendación:** Vapi para Fase 1 ($0.05/min + BYO Claude API key + HIPAA tier predictable) con MVP de 1 cuestionario piloto (PHQ-9). Status: research-pending-approval (Jorge debe aprobar antes de codear 10-13 h reales).
